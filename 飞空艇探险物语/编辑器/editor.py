...
X_TYPE_OFFSETS = {
    ...
    "XString": {"bitmask": 0, "original": 4, "value": 8, "buffer": 12, "hashCode": 16},
}
...
class MemoryEditor:
    """内存编辑器主类，负责字段管理、读写和 UI 交互"""
    SHOW_EDIT_BUTTONS = False   # 设为 False 可隐藏按钮
    ...
    def __init__(self, parent, table_name, table_config_path):
        ...
        self.fields_def = []
        self.offset_chains = []  # 与 fields_def 索引对齐的整数偏移列表
        for f in raw_fields:
    ...
    def _write_xsecure(self, addr, xtype, value):
        """写入 XType 结构体，更新 original 和 value（加密）"""
        offsets = X_TYPE_OFFSETS[xtype]
    ...
    def _read_list(self, addr, inner_type):
        """读取 List<T> 容器 - 一级偏移：直接 list_ptr + 0x8 得到数组指针"""
    ...
    def _read_value_at(self, addr, ftype):
        """根据类型读取内存值，支持标量、数组、Vector、List、XType"""
    ...
    def _write_bool_field(self, addr, fdef_idx, var):
        ...
        try:
            ...
            label = get_display_label(fdef, self.offset_chains[fdef_idx])
            self.status.config(text=f"✔ {label}: {bool_val} → {verify}", foreground="green")
        except Exception as e:
            self.status.config(text=f"✘ {fdef['name']}: 写入失败 ({e})", foreground="red")
    ...
    def _create_scalar_editor(self, parent, obj_addr, fdef_idx, value, ftype, path, row_idx, col_offset=0, index=None):
        """创建标量编辑器（用于数组/容器内部）"""
    ...
    def _create_array_container(self, parent, obj_addr, fdef_idx, value_list, ftype, path, row_idx, col_offset=0, index=None):
        """创建可展开的数组/容器 UI"""
            ...
            if container.expanded:
                ...
                for idx, item in enumerate(value_list):
                    ...
                    sub_row = self._create_editor_for_value(
                        editor_frame, obj_addr, fdef_idx, item, sub_ftype, sub_path, sub_row, col_offset + 1, idx
                    )
    ...
    def display_fields(self, addr, obj_id, fields):
        ...
        for i, fdef in enumerate(self.fields_def):
            ...
            if isinstance(val, list):
                self._create_array_container(value_frame, addr, i, val, ftype, [], 0)
            else:
                self._create_scalar_editor_for_field(value_frame, addr, i, val, ftype)

            row += 1

    def _create_scalar_editor_for_field(self, parent, addr, fdef_idx, value, ftype):
        """创建字段的标量编辑器 - 固定输入框宽度，不拉伸"""
        fdef = self.fields_def[fdef_idx]
        is_err = isinstance(value, str) and value.startswith("ERR")
        if ftype == "bool":
            var = tk.StringVar(value="True" if value else "False")
    ...
    def _rename_field_display(self, field_name):
        ...
        new = new.strip()
        f["display"] = new
        save_json(self.fields_file, {"fields": self.fields_def})  # 只保存 fields_def，不含额外键
