...
优化（v5.36）：分页表不再每次重载都跳回第一页，且只读当前页用到的字段。
  ...
  并且刷新/切换对象/切表都保留当前页码。
新增（v5.37）：字段配置可加可选的 "ui" 规则表，把"显示 / 编辑方式"配到容器里的任意一格上。
  规则形如 {"at": [8], "bit_edit": true, "bit_count": 16}：at 是从字段值出发的路径，
  数组/List/Vector 用下标、字典用 {"key": 值}、自定义对象用字段名，"*" 通配一层；
  规则可带 display / hidden / enum / bit_edit / bit_count / index_labels / key_labels，
  逐层前缀匹配、越深越优先，所以 {"at": [8], "index_labels": [...]} 会连第 8 格的子孙一起管。
  规则只加不减：没写 "ui" 的字段行为与以前完全一致。
  例：存档 Property.ints 的第 8 格（FLAG）配 {"at": [8], "bit_edit": true} 就能逐位开关；
  脚本行 [指令号, 参数...] 配 {"at": ["*", 0], "enum": {...}} 只给指令号加下拉，参数仍是输入框。
  （原 v5.37 试做过的 enum_index 已废弃并删除，等价写法就是上面这条 at 规则。）
依赖：
  - pymem：用于进程内存操作
...
class MemoryEditor:
    ...
    def _get_index_label(self, fdef_idx, index, path):
        ...
        标签来源（按优先级）：
          1. 该格"父级槽位"的 index_labels —— 字段自身的只在第一层生效（今天的行为），
             若第一层配了 ui 规则，则规则里的 index_labels 覆盖它，且对更深层同样有效；
          2. 该格自己 ui 规则里的 display；
          3. [下标]
        """
        if index is None:
            return ""
        parent = list(path or [])[:-1]
        over = self._ui_overrides(fdef_idx, parent)
        if "index_labels" in over:
            labels = over["index_labels"] or []
        elif not parent:
            labels = self.fields_def[fdef_idx].get("index_labels", []) if 0 <= fdef_idx < len(self.fields_def) else []
        else:
            labels = []
        if isinstance(labels, list) and 0 <= index < len(labels) and labels[index]:
            return labels[index]
        disp = self._ui_overrides(fdef_idx, path).get("display")
        if disp:
            return str(disp)
        return f"[{index}]"
    ...
    def _create_scalar_editor(self, parent, obj_addr, fdef_idx, value, ftype, path, row_idx=0, col_offset=0, index=None):
        ...
        ui = self._ui_for(fdef_idx, path)

        # 位编辑：字段级 bit_edit 走主表分支，这里负责"容器里的某一格"（如 ints[8] = FLAG）
        if ui.get("bit_edit") and ftype in ("byte", "sbyte", "int", "long") and not is_err:
            self._create_bit_editor_container(parent, obj_addr, fdef_idx, value, ftype,
                                              path=path, row=row_idx, col=col, ui=ui)
            return row_idx + 1

        enum_def = ui.get("enum") or None
        ...
        def toggle():
            if container.expanded:
                ...
                for idx, item in enumerate(value_list):
                    ...
                    # ui 规则可以把某一格隐藏掉（大数组只留关心的几格）
                    if self._ui_for(fdef_idx, sub_path).get("hidden"):
                        continue
    ...
    # ========== 按位编辑 ==========
    @staticmethod
    def _mask_unsigned(value, ftype):
        """按类型截成无符号，便于按位展示与运算"""
        if ftype in ("byte", "sbyte"):
            return value & 0xFF
        if ftype == "int":
    ...
    @staticmethod
    def _to_signed(value, ftype):
        """按类型把无符号值折回有符号（写回内存时用）"""
        if ftype == "sbyte" and value >= 0x80:
            return value - 0x100
        if ftype == "int" and value >= 0x80000000:
            return value - 0x100000000
        if ftype == "long" and value >= 0x8000000000000000:
            return value - 0x10000000000000000
        return value

    @staticmethod
    def _hex_width(ftype):
        return {"byte": 2, "sbyte": 2, "int": 8, "long": 16}.get(ftype, 8)

    def _create_bit_editor_container(self, parent, obj_addr, fdef_idx, value, ftype,
                                     path=None, row=0, col=0, ui=None):
        """创建按位编辑的容器（低位到高位，选中即写入）。

        path 为空 = 字段本身；path 非空 = 数组 / List / Vector 里的某一格
        （这样 Property.ints[8] 这种"容器里的 flag"也能逐位编辑）。
        位宽取 ui["bit_count"]（缺省 8/8/32/64，且不超过类型上限），位名取 ui["index_labels"]。
        value 只用于调用方已经读到的值，这里不显示。
        """
        ui = ui if ui is not None else self._ui_for(fdef_idx, path)
        type_bits = {"byte": 8, "sbyte": 8, "int": 32, "long": 64}[ftype]
        bit_count = ui.get("bit_count")
        if not isinstance(bit_count, int) or bit_count <= 0:
            bit_count = type_bits
        bit_count = min(bit_count, type_bits)
        labels = ui.get("index_labels") or []

        fdef = self.fields_def[fdef_idx]
        label = get_display_label(fdef, self.offset_chains[fdef_idx])
        if path:
            label += "".join("[%s]" % p for p in path)

        def read_current():
            """读当前值：字段走 _read_field，容器里的格子走路径地址"""
            if path:
                addr = self._get_element_address(obj_addr, fdef_idx, path)
                return self._read_value_at(addr, ftype)
            return self._read_field(obj_addr, fdef_idx)

        def write_value(new_val):
            """写回：字段走 _write_field_value，容器里的格子复用数组元素写入"""
            if path:
                self._write_array_element(fdef_idx, path, ftype, tk.StringVar(value=str(new_val)))
            else:
                self._write_field_value(obj_addr, fdef_idx, new_val)

        container = ttk.Frame(parent)
        container.grid(row=row, column=col, sticky='w', pady=1)

        preview_frame = ttk.Frame(container)
        preview_frame.grid(row=0, column=0, sticky='w')
        preview_label = ttk.Label(preview_frame, text="位编辑", foreground="blue")
        preview_label.grid(row=0, column=0, sticky='w', padx=5)
        expand_btn = ttk.Button(preview_frame, text="▶ 展开", width=6)
        expand_btn.grid(row=0, column=1, sticky='w', padx=5)
        ...
        container.obj_addr = obj_addr
        container.fdef_idx = fdef_idx
        container.ftype = ftype
        container.bit_count = bit_count
        container.bit_vars = []  # 保存所有位的 StringVar
        container.read_fn = read_current
        container.write_fn = write_value
        ...
        def toggle():
            if container.expanded:
                ...
                try:
                    ...
                    current_val = self._mask_unsigned(current_val, ftype)
                except Exception as e:
                ...
                for bit_idx in range(container.bit_count):
                    bit_label = labels[bit_idx] if bit_idx < len(labels) and labels[bit_idx] else f"[{bit_idx}]"
                    ttk.Label(editor_frame, text=bit_label, foreground="gray").grid(row=sub_row, column=0, sticky='w', padx=2)
    ...
    def _on_bit_changed(self, fdef_idx, bit_idx, container):
        """位变化时自动写入指定位，并刷新所有位与当前值（字段 / 容器元素通用）"""
        if self.current_edit_addr is None:
            return
        try:
            ftype = container.ftype
            current = container.read_fn()
            if isinstance(current, str) and current.startswith("ERR"):
                raise ValueError(current)
            current = self._mask_unsigned(current, ftype)
            # 获取新位值
            new_bit = 1 if container.bit_vars[bit_idx].get() == "1" else 0
            # 更新指定位
            if new_bit:
                new_val = current | (1 << bit_idx)
            else:
                new_val = current & ~(1 << bit_idx)
            container.write_fn(self._to_signed(new_val, ftype))
            verify = container.read_fn()
            if isinstance(verify, str) and verify.startswith("ERR"):
                raise ValueError(verify)
            verify_val = self._mask_unsigned(verify, ftype)
            # 刷新所有位的显示
            for i, var in enumerate(container.bit_vars):
                var.set("1" if (verify_val >> i) & 1 else "0")
            self.status.config(text=f"✔ {container.label} 位{bit_idx}: {new_bit}", foreground="green")
        except Exception as e:
    ...
    def _build_field_page(self, container, addr, fields, page_indices):
        ...
        for i in page_indices:
            ...
            try:
                ...
                is_err = isinstance(val, str) and val.startswith("ERR")
                # 位编辑分支（字段自身 bit_edit，或 ui 规则里 at:[] 的 bit_edit）
                ui_top = self._ui_for(i, [])
                if ui_top.get("bit_edit") and ftype in ("byte", "sbyte", "int", "long") and not is_err:
                    self._create_bit_editor_container(hframe, addr, i, val, ftype, ui=ui_top)
                else:
    ...
    def _get_field_enum(self, fdef_idx, path=None):
        """取该槽位生效的 enum 定义（字段自身配的，或 ui 规则里配的），没有则 None"""
        return self._ui_for(fdef_idx, path).get("enum") or None

    # ---------- ui 规则（容器内任意一格的显示/编辑方式） ----------
    @staticmethod
    def _ui_path_match(at, prefix):
        """规则路径 at 是否等于 prefix（长度必须相同，"*" 通配一层）"""
        if len(at) != len(prefix):
            return False
        for a, p in zip(at, prefix):
            if a == "*":
                continue
            if a != p:
                return False
    ...
    def _ui_overrides(self, fdef_idx, path):
        """只取 "ui" 规则贡献的键。

        逐层前缀匹配：第 d 层的槽位会吃到所有 at == path[:d] 的规则，越深越优先，
        同一层按书写顺序后者覆盖前者。没有 ui 键则返回 {}（等于"什么都不改"）。
        路径元素约定：数组/List/Vector 用下标 int，字典用 {"key": k}，自定义对象用字段名 str。
        """
        if not (0 <= fdef_idx < len(self.fields_def)):
            return {}
        rules = self.fields_def[fdef_idx].get("ui")
        if not rules:
            return {}
        path = list(path or [])
        over = {}
        for depth in range(0, len(path) + 1):
            ...
            for rule in rules:
                ...
                if not isinstance(at, list):
                    continue
                if self._ui_path_match(at, prefix):
                    for k, v in rule.items():
                        if k != "at":
                            over[k] = v
        return over

    def _ui_for(self, fdef_idx, path=None):
        """某槽位实际生效的 UI 配置 = 字段自身配置 + 该路径上的 ui 规则覆盖。

        没有 ui 规则的字段直接返回自身配置，调用方读 enum / bit_edit / index_labels
        等键的结果与升级前完全一致。
        """
        if not (0 <= fdef_idx < len(self.fields_def)):
            return {}
        fdef = self.fields_def[fdef_idx]
        over = self._ui_overrides(fdef_idx, path)
        if not over:
            return fdef
        ui = dict(fdef)
        ui.update(over)
    ...
    def _create_scalar_editor_for_field(self, parent, fdef_idx, value, ftype):
        ...
        fdef = self.fields_def[fdef_idx]
        is_err = isinstance(value, str) and value.startswith("ERR")
        ui = self._ui_for(fdef_idx, [])
        if ui.get("enum") and ftype in ("int", "byte", "sbyte", "long") and not is_err:
            self._create_enum_combobox(
